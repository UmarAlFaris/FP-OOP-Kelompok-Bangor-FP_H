...
class FightBase(ScreenBase):
    ...
    def _load_enemy(self, index):
        ...
        if os.path.isfile(sheet_path):
            ...
            p = os.path.join(os.path.dirname(__file__), '..', '..', 'Testing', 'assets', name, 'Idle.png')
        ...
        class TurnBasedGrid(ScreenBase):
            ...
            def __init__(self, manager, screen_size, stages: list[str], next_scene=None, forced_inference=None):
                super().__init__(manager, screen_size)
                # dynamic import of Code/main.py to reuse helpers and assets
                import importlib.util
                repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
                codemain_path = os.path.join(repo_root, 'Code', 'main.py')
                try:
                    spec = importlib.util.spec_from_file_location('codemain', codemain_path)
                    codemain = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(codemain)
                except Exception:
                    codemain = None

                # fallback constants
                self.GRID_W = getattr(codemain, 'GRID_W', 8)
                self.GRID_H = getattr(codemain, 'GRID_H', 6)
                ...
                # player unit
                Unit = getattr(codemain, 'Unit', None)
                if Unit:
                    self.player = Unit(1, self.GRID_H//2, getattr(codemain, 'PLAYER_MAX_HP', 20), getattr(codemain, 'PLAYER_ATK', 5), 'PLAYER', mana=getattr(codemain, 'PLAYER_MANA', 100), mana_regen=getattr(codemain, 'PLAYER_MANA_REGEN', 5))
                else:
                    self.player = type('U', (), {'x':1,'y':self.GRID_H//2,'hp':20,'max_hp':20,'atk':5,'team':'PLAYER','alive':True,'mana':100,'mana_regen':5, 'pos': lambda self:(self.x,self.y)})()
                ...
                if codemain:
                    try:
                        idle_frames = []
                        for i in range(1,9):
                            p = codemain.asset_path(f'player/idle{i}.png')
                            if os.path.isfile(p): idle_frames.append(p)
                        if idle_frames:
                            self.player_idle_anim = codemain.AnimatedSprite(idle_frames, (self.TILE+30, self.TILE+30))
                        # load enemy frames (best-effort)
                        try:
                            sheet = pygame.image.load(codemain.asset_path('zombie/Idle.png')).convert_alpha()
                            n = getattr(codemain, 'ZOMBIE_Y_OFFSET', 6)
                        except Exception:
                            sheet = None
                        # use codemain loading for other frames if available
                        if hasattr(codemain, 'zombie_frames'):
                            self.zombie_frames = getattr(codemain, 'zombie_frames')
                        else:
                            try:
                                sheet = pygame.image.load(codemain.asset_path('zombie/Idle.png')).convert_alpha()
                                n_frames = 6
                        ...
                        # skeleton
                        try:
                            skel_sheet = pygame.image.load(codemain.asset_path('skeleton/Idle.png')).convert_alpha()
                            n_skel = 7
                        ...
                        # enderman
                        try:
                            end_sheet = pygame.image.load(codemain.asset_path('enderman/Idle.png')).convert_alpha()
                            n_end = 14
                        ...
                        # boss
                        try:
                            boss_sheet = pygame.image.load(codemain.asset_path('boss/Idle.png')).convert_alpha()
                            n_boss = 8
            ...
            def spawn_enemy(self, index):
                ...
                try:
                    ...
                    codemain = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(codemain)
                    UnitCls = getattr(codemain, 'Unit', None)
                except Exception:
                    codemain = None
                if UnitCls:
                    self.enemy = UnitCls(ex, ey, ehp, eatk, 'ENEMY', mana=emana, mana_regen=5 if etype in ('Enderman','Boss') else 0)
                    ...
                    self.enemy = type('E', (), {'x':ex,'y':ey,'hp':ehp,'max_hp':ehp,'atk':eatk,'team':'ENEMY','alive':True,'mana':emana,'mana_regen':5 if etype in ('Enderman','Boss') else 0,'pos': lambda self:(self.x,self.y)})()
                # update units
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_m and self.turn == 'PLAYER':
                        ...
                        try:
                            ...
                            codemain = importlib.util.module_from_spec(spec)
                            spec.loader.exec_module(codemain)
                            self.move_targets = codemain.bfs_reachable(self.player.pos(), getattr(codemain, 'MOVE_RANGE', 1), {self.enemy.pos()})
                        except Exception:
                            self.move_targets = set()
                    ...
                    if event.key == pygame.K_h and self.turn == 'PLAYER':
                        # heal
                        if getattr(self.player, 'mana', 0) >= getattr(self.player, 'max_mana', 100):
                            self.player.mana -= getattr(self, 'PLAYER_HEAL_COST', 50)
                            self.player.hp = min(self.player.max_hp, self.player.hp + getattr(self, 'PLAYER_HEAL_AMOUNT', 10))
                            self.message = f'Player heal +{getattr(self, "PLAYER_HEAL_AMOUNT",10)}.'
                            self.end_turn()
                        else:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.button == 1:
                        ...
                        if self.turn == 'PLAYER' and my < self.GRID_H * self.TILE:
                            ...
                            elif abs(self.player.x - gx) + abs(self.player.y - gy) <= getattr(self, 'MOVE_RANGE', 1) and self.unit_at((gx,gy)) is None:
            ...
            def enemy_action(self):
                # copy Code/main.py's enemy_action logic (uses fuzzy)
                try:
                    import importlib.util
                    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
                    codemain_path = os.path.join(repo_root, 'Code', 'main.py')
                ...
                for u in self.units:
                    ...
                    if getattr(u, 'team', None) == 'PLAYER' and self.player_idle_anim:
                        ...
                        if etype == 'Zombie' and self.zombie_frames:
                            ...
                            rect = frame.get_rect(center=(cx, cy - getattr(self, 'ZOMBIE_Y_OFFSET', 27)))
                            ...
                            rect = frame.get_rect(center=(cx, cy - getattr(self, 'SKELETON_Y_OFFSET', 27)))
                            surface.blit(frame, rect)
                        elif etype == 'Enderman' and self.enderman_frames:
                            frame = self.enderman_frames[self.enderman_anim_index % len(self.enderman_frames)]
                            rect = frame.get_rect(center=(cx + getattr(self, 'ENDERMAN_X_OFFSET', 20), cy - getattr(self, 'ENDERMAN_Y_OFFSET', 22)))
                            surface.blit(frame, rect)
                        elif etype == 'Boss' and self.boss_frames:
                            frame = self.boss_frames[self.boss_anim_index % len(self.boss_frames)]
                            rect = frame.get_rect(center=(cx, cy - getattr(self, 'BOSS_Y_OFFSET', 1)))
                            surface.blit(frame, rect)
                        else:
...
class TurnBasedGrid(ScreenBase):
    ...
    def __init__(self, manager, screen_size, enemies: list[dict] = None, stages: list[str] = None, next_scene=None, forced_inference=None):
        super().__init__(manager, screen_size)
        # mirror Code/main.py defaults: GRID_W=8, GRID_H=6 and UI panel height 120
        self.grid_w = 8
        self.grid_h = 6
        usable_h = self.screen_height - 120
        self.tile = min(self.screen_width // self.grid_w, usable_h // self.grid_h)
        ...
        player_paths = [
            os.path.join(repo, 'Testing', 'Assets', 'player'),
            os.path.join(repo, 'Code', 'assets', 'player')
        ]
        ...
        for e in self.enemies:
            ...
            # Try Testing/Assets first, then Code/assets
            enemy_paths = [
                os.path.join(repo, 'Testing', 'Assets', name),
                os.path.join(repo, 'Code', 'assets', name)
            ]
        ...
        # import fuzzy from Code package
        try:
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
            ...
            try:
                from Code import fuzzy as fuzzy_mod
            except Exception:
                import importlib
                fuzzy_mod = importlib.import_module('Code.fuzzy') if 'Code.fuzzy' in sys.modules or os.path.isdir(os.path.join(repo_root, 'Code')) else None
            self.fuzzy = fuzzy_mod
        except Exception:
            self.fuzzy = None
